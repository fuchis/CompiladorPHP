        ...
        if(lex.length>=1) {
            switch(pila.length) {
                case 0: // Pila vacia, esperando que busque algun token, comienzo del analisis sintactico
                    ...
                    break;   
                case 1:
                    if(inicioSwitch) { //Si previamente se detecto un SWITCH
                    ...
                    // }else if(lexema.match(tkn_del)){ //si no es (|ID, buscar un DEL ;
                    //     pila.push(lexema);
                    //     if((index+1)<lex.length){ //si el indice + 1 es menor al tamanio del arrelgo(osea no es el fin)
                    //         let llaveDer = busquedaObjetoPorKey(lexTokOriginal, "lexema", "}"); //buscar que el sigueinte token sea un }
                    ...
                    break;
                case 2:
                    if(lexema.match(tkn_id)){ // SWITCH ( ID
                        if((index+1) == lex.length) {
                            pila.push(lexema);
                            if(inicioSwitch){
                                erroresSintacticos.push("Se esperaba un )");
